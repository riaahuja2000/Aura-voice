...
agent_communication:
  ...
  - agent: "main"
    message: "LIVING ORACLE OS UPGRADE (build 3). Backend: new /app/backend/oracle_brain.py + rewritten POST /api/voice/consult — invisible engine router (returns engine/mode/action fields), context memory via db.voice_turns, one-breath clarification (mode=clarify), council mode, spoken commands ('calm me now'→instant rescue script no-LLM, 'save this moment', 'play my guidance', 'where did this answer come from', 'challenge this answer', 'forget this conversation', 'delete my voice history', \"don't remember personal details\"). New POST /api/voice/refine {direction: deeper|shorter|practical|alternative|challenge|source}. New GET /api/owner/voice-log (owner only). Frontend: home.tsx rewritten with gesture system (hold=talk, tap=pause, double-tap=replay, swipes=refine, two-finger=whisper), captions toggle (eye icon), first-launch AI disclosure. Owner console gained Voice Oracle Log section. Main agent already smoke-tested: consult (dream engine), rescue, refine:shorter, save, owner voice-log all 200 OK; home orb + captions toggle render fine."
